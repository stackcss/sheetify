...
if (!files.length) help()
else {
  var bundler = sheetify(files[0])
  ...
  bundler.bundle(opts, function (err, data) {
    if (err) throw err
...
function help () {
  ...
  process.stdout.write(usage.replace(/\$0/g, $0))
}
...
function convertAliases (argv, aliases, defaults) {
  Object.keys(aliases).forEach(function (alias) {
    ...
    argv[key] = argv[key] ? value.concat(argv[key]) : value
    delete argv[alias]
  })
